...
func (optr *Operator) syncMachineConfigDaemon(config *renderConfig) error {
	...
	if apierrors.IsNotFound(err) {
		csBytes, err := renderAsset(config, "manifests/machineconfigdaemon/cookie-secret.yaml")
		if err != nil {
			return err
		}
		cs := resourceread.ReadSecretV1OrDie(csBytes)
		_, _, err = resourceapply.ApplySecret(optr.kubeClient.CoreV1(), cs)
		if err != nil {
			return err
		}
	} else if err != nil {
